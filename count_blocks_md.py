...
def read_log_filter(datalog, common):
  ...
  with open(datalog, "r") as fin:
    for line in fin:
      ...
      counts = list(map(lambda x: float(x) <= FILTER, numbers))
      listCounts.append(counts)
  step_contact_bool = list(map(max, listCounts))
  count_col = list(zip(*listCounts)) #*sign to unpack nested row list/tuple into columns 
...
if __name__ == '__main__':
  if len(sys.argv) < 4:
    print("python countResidue.py label_by_block.txt datalog output.txt")
    sys.exit()
